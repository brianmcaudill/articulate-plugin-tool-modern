...
const initStorylineInterface = () => {
    ...
    const ribbonInterface = initStorylineRibbonExtended();
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.ctrlKey && e.code === 'KeyS') {
            ribbonInterface.handleQuickSave();
            e.preventDefault();
        }

        // Ctrl+Z for undo
        if (e.ctrlKey && e.code === 'KeyZ' && !e.shiftKey) {
            ribbonInterface.handleUndo();
            e.preventDefault();
        }
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimeout = setTimeout(() => {
            ribbonInterface.render(); // Re-render ribbon to adjust layout
        }, 250);
    });
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        if (shouldUpdate) {
            ribbonInterface.showContextualTabs(document.querySelector('[data-sl-type]')?.dataset.slType);
        }
    });
    ...
    document.addEventListener('mousedown', (e) => {
        ...
        if (target) {
            ...
            ribbonInterface.showContextualTabs(target.dataset.slType);
        }
    });
...
(() => {
    ...
    const init = async () => {
        try {
            ...
            const ribbonInterface = initStorylineInterface();
